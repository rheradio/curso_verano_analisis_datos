...
Un grupo de investigadores ha recolectado evaluaciones subjetivas de expertos en literatura, historia y cultura manchega sobre cuatro pueblos candidatos a ser el famoso "lugar de La Mancha de cuyo nombre no quiero acordarme". Los criterios de evaluación son:
...
$$\sqrt{(x_1-x_2)^2+(y_1-y_2)^2}$$ {#eq-distancia} La siguiente tabla recoge las distancias de los principales pueblos de Castilla La Mancha:
...
Aunque se están estudiando cuatro pueblos, la intuición nos dice que tiene que ser Argamasilla de Alba o Villanueva de los Infantes.

La tabla @tbl-prueba1 muestra las opiniones de los expertos sobre qué pueblo es el verdadero lugar de la mancha:
...
Como los investigadores tenían olvidadilla la historia de El Quijote, se la volvieron a leer. La tabla @tbl-prueba2 muestras las opciones de los expertos sobre la idoneidad de Villanueva como el pueblo elegido antes y después de leer El Quijote:
...
Como les piden a los investigadores un análisis más riguroso para determinar cuál es el pueblo buscado, se recogió la opinión de todos los expertos sobre todos los pueblos (@tbl-prueba3).

¿Existen diferencias significativas entre los pueblos? ¿cuáles?
...
Para poder tomar una decisión final, se hizo una media las opiniones de los expertos para cada criterio (Idealismo, ambiente rural, tradición caballeresca, probabilidad histórica). La tabla @tbl-prueba4 muestra los resultados:
...
### Analizando los datos {#sec-analisis}

Una vez obtenidos los datos, se han realizado test no paramétricos para analizarlos y poder responder a las preguntas. Las siguientes secciones detallan los test realizados:

#### Prueba 1: Test de Mann-Whitney {#sec-analisis-p1}

Puesto que se quiere comparar las diferencias entre las puntuaciones de dos muestras (una por cada pueblo, que son distintos), y no se conoce la distribución de los datos, el test más adecuado es el de **Mann-Whitney**:
...
En este caso, se quiere comparar las diferencias de las medidas (opiniones) entre dos muestras pareadas (se mide antes/después de leer sobre los mismos expertos), y no se conoce la distribución de los datos, el test más adecuado es el de **Rangos Signados de Wilcoxon**:
...
En este caso, se quiere comparar las diferencias entre cuatro grupos (pueblos) donde las observaciones corresponden a individuos (expertos) distintos, y no se conoce la distribución de los datos, el test más adecuado es el de **Kruskal Wallis**:
...
En este caso, se quiere comparar las diferencias entre medidas (criterios) de los mismos individuos (pueblos) y no se conoce la distribución de los datos, el test más adecuado es el de **Friedman**:
...
En @sec-intro, se describió el problema, que era encontrar el pueblo del D.Quijote.

En @sec-mapa, se mostraron los mapas actual y antiguo (@fig-mapa). También se añadió la donde se mostraban las coordenadas y las distancias de los pueblos posibles a los cuatro pueblos descritos en la novela que visió D. Quijote (El Toboso, Sierra Morena, Puerto Lápice y *punto Tarfe).* Las distancias se han calculado según la @eq-distancia.

A continuación se muestra las conclusiones obtenidas en el análisis descriptivo y en los test no paramétricos.

### Análisis descriptivo:

Del [**análisis descriptivo**]{.underline} se observa que:
...
De las realización de los [**test no paramétricos**]{.underline} se deduce:
